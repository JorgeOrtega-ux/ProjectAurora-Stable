...
elseif ($CURRENT_SECTION === 'search') { $SECTION_FILE_NAME = 'app/search-results'; } 
elseif (strpos($CURRENT_SECTION, 'register/') === 0 || $CURRENT_SECTION === 'register') { $SECTION_FILE_NAME = 'auth/register'; } 
elseif ($CURRENT_SECTION === 'forgot-password') { $SECTION_FILE_NAME = 'auth/forgot-password'; } 
elseif ($CURRENT_SECTION === 'reset-password') { $SECTION_FILE_NAME = 'auth/reset-password'; } 
elseif ($CURRENT_SECTION === 'login') { $SECTION_FILE_NAME = 'auth/login'; } 
elseif (strpos($CURRENT_SECTION, 'settings/') === 0) { $SECTION_FILE_NAME = $CURRENT_SECTION; } 
elseif (strpos($CURRENT_SECTION, 'admin/') === 0) { $SECTION_FILE_NAME = $CURRENT_SECTION; } 
elseif (in_array($CURRENT_SECTION, $appSections)) { $SECTION_FILE_NAME = 'app/' . $CURRENT_SECTION; } 
elseif (in_array($CURRENT_SECTION, $systemSections)) { $SECTION_FILE_NAME = 'system/' . $CURRENT_SECTION; } 
else { $SECTION_FILE_NAME = 'system/404'; }
...
if (!$isLoggedIn && !in_array($CURRENT_SECTION, $publicSections) && $CURRENT_SECTION !== '404') { header("Location: " . $basePath . "login"); exit; }
if ($isLoggedIn && in_array($CURRENT_SECTION, $publicSections) && $CURRENT_SECTION !== 'status-page') { header("Location: " . $basePath); exit; }
