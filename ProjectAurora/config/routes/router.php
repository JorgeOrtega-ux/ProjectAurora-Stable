...
// [MODIFICADO] Variables de contexto para el Chat (Comunidad, Privado y Canal)
$activeContextUuid = null;
$activeChannelUuid = null; // [NUEVO] Variable para el canal
$activeContextType = null; // 'community' o 'private'

// 1. Detectar Comunidad (/c/uuid o /c/uuid/channel_uuid)
if (preg_match('/^c\/([a-zA-Z0-9-]+)(?:\/([a-zA-Z0-9-]+))?$/', $requestUri, $matches)) {
    $CURRENT_SECTION = 'main';
    $activeContextType = 'community';
    $activeContextUuid = $matches[1];
    
    // Si existe el segundo grupo capturado, es el canal
    if (isset($matches[2]) && !empty($matches[2])) {
        $activeChannelUuid = $matches[2];
    }
}
