    ...
    if (window.ACTIVE_CHAT_UUID) {
        ...
        if (itemData) {
            ...
            if (itemData.type === 'community') {
                 // Cargar canales usando el nuevo manager
                 ...
                 // [MODIFICADO] Prioridad: URL > Default > Fallback
                 if (window.ACTIVE_CHANNEL_UUID) {
                     targetCh = channels.find(c => c.uuid === window.ACTIVE_CHANNEL_UUID);
                 }
                 
                 // Si no hay canal en URL o es inválido, probar default
                 ...
                 if (targetCh) {
                     ...
                     // Actualizar variable global para que coincida
    ...
    document.getElementById('my-communities-list')?.addEventListener('click', async (e) => {
        ...
        if (item) {
            ...
            if (type === 'community') {
                ...
                // [NUEVO] Priorizar el canal activo si ya estamos ahí (deep link)
...
export function initCommunitiesManager() {
    ...
    // Iniciar listener del módulo Info
